...
| 云中继 | 否 | `https://keeper.kayz.com/wecom` |
...
3. **配置回调 URL**：在「接收消息」设置中填写 `https://keeper.kayz.com/wecom` 并保存
...
2. URL 填写：`https://keeper.kayz.com/wecom`
3. Token 和 EncodingAESKey 填写之前记录的值
4. 点击「保存」

企业微信会向 `https://keeper.kayz.com/wecom` 发送验证请求，云中继服务器使用你发送的凭据完成验证。如果看到保存成功，说明验证通过。
...
┌──────────────┐     ┌─────────────────────────────┐     ┌──────────────────┐
│  企业微信     │     │    云中继服务器              │     │   本地客户端      │
│  用户消息     │ ──▶ │  keeper.kayz.com/wecom       │ ──▶ │  lingti-bot relay │
│              │     │  (验证回调、转发消息)         │ WS  │  (AI 处理)        │
└──────────────┘     └─────────────────────────────┘     └──────────────────┘
       ▲                          │                              │
       │                          │ ◀─────────────────────────────┘
...
4. **检查网络连接**：确保能连接到 `wss://keeper.kayz.com/ws`
