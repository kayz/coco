...
云中继是 lingti-bot 提供的一种**免公网服务器**接入方案。传统的聊天平台 Bot 接入需要一台拥有公网 IP 的服务器来接收回调消息，而云中继通过在云端部署一个中转服务器（`keeper.kayz.com`），让用户在本地（笔记本、家庭网络、公司内网）即可完成 Bot 的全部功能。
...
┌──────────┐    ┌────────────────────────┐    ┌─────────────────────────┐
│          │    │   云中继服务器           │    │   用户本地环境           │
│ 企业微信  │───▶│   keeper.kayz.com       │◀───│   lingti-bot relay      │
│ 服务器    │◀───│                        │───▶│                         │
│          │    │  ┌──────────────────┐   │    │  ┌───────────────────┐  │
...
| **云中继服务器** | `keeper.kayz.com`，负责接收回调、转发消息、代理 API 调用 | 公网（IP: `106.52.166.51`） |
...
- **URL** 填写：`https://keeper.kayz.com/wecom`
...
| **需要公网服务器** | 否 | 是 |
| **需要域名/证书** | 否 | 可选 |
| **回调 URL** | `https://keeper.kayz.com/wecom` | `http://YOUR_IP:PORT/wecom/callback` |
...
3. **检查网络** — 确保能连接到 `wss://keeper.kayz.com/ws`
