...
All configuration in lingti-bot follows a strict three-tier priority: **CLI flags > environment variables > config file (`~/.coco.yaml`)**. See [CONFIGURATION.md](CONFIGURATION.md) for full details.
...
3. Fall back to `~/.coco.yaml` config file (lowest priority)
