...
- All platform credentials can be saved via `lingti-bot onboard` and stored in `~/Library/Preferences/Lingti/bot.yaml` (macOS) or `~/.config/coco/bot.yaml` (Linux).
- 所有平台凭证可通过 `lingti-bot onboard` 保存到 `~/Library/Preferences/Lingti/bot.yaml`（macOS）或 `~/.config/coco/bot.yaml`（Linux）。
