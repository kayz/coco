...
using your local Claude API key.
...
User Flow:
  ...
  4. Messages are processed locally with your Claude API key
...
Required:
  ...
  --api-key     Claude API key (or ANTHROPIC_API_KEY env)
...
func init() {
	...
	relayCmd.Flags().StringVar(&relayAPIKey, "api-key", "", "Claude API key (or ANTHROPIC_API_KEY env)")
	relayCmd.Flags().StringVar(&relayBaseURL, "base-url", "", "Custom API base URL (or ANTHROPIC_BASE_URL env)")
	relayCmd.Flags().StringVar(&relayModel, "model", "", "Claude model to use (or ANTHROPIC_MODEL env)")
}
...
func runRelay(cmd *cobra.Command, args []string) {
	...
	if relayAPIKey == "" {
		fmt.Fprintln(os.Stderr, "Error: Claude API key is required (--api-key or ANTHROPIC_API_KEY env)")
		os.Exit(1)
	}
	...
	log.Println("Processing messages with your local Claude API key.")
